...
export default function AddTestModal({ onClose, onAddTest }: AddTestModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    console.log('AddTestModal submitting formData:', JSON.stringify(formData, null, 2));
    try {
      await onAddTest(formData);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-sm max-w-4xl w-full max-h-[95vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-2xl font-semibold text-black">Add New Test</h2>
          <button
            onClick={onClose}
